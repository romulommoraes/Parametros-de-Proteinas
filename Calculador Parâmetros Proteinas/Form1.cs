...
namespace ParametrosProteinas
{
    public partial class Form1 : Form
    {
        ...
        private void btn_contar_Click(object sender, EventArgs e)
        {
            ...
            outputCSV.Add("Id\tNºAa\tmW(D)\tGravy\tSequência");
            for (int i = 0; i < heads.Count; i++)
            {
                Proteina p = new Proteina(heads[i], prots[i], 0, 0, 0);
                p.nA = p.calcNa();
                p.mW = p.calcMW();
                p.gravy = p.calcGravy();
                total.Add(p);
            ...
            {
                String[] itemLinha = new string[] { item.head, item.nA.ToString(), item.mW.ToString(), item.gravy.ToString() };
